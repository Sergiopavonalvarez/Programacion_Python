...
def codigoBoton():
    miNombre=cuadroNombre.get()
    miApellido=cuadroApellido.get()
    miDireccion=cuadroDireccion.get()
    miPassword=cuadroPassword.get()
    miComentario=comentariosText.get("1.0", END)
    print(f"Nombre: {miNombre}\nApellido: {miApellido}\nDireccion: {miDireccion}\nPassword: {miPassword}\nComentario: {miComentario}")

botonEnvio=Button(miFrame, text="Enviar", command=codigoBoton)
botonEnvio.grid(row=5,column=1, sticky="e")
botonEnvio.config(padx=50, pady=5)
